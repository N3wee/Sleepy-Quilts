...
def main():
    ...
    while True:
        ...
        # Deduct today's orders before checking stock for tomorrow
        deduct_today_material_usage(today)

        # Check stock status based on tomorrow's orders
        if check_if_orders_exist(tomorrow):
            stock_status = check_stock_for_tomorrow(tomorrow)
            print(f"\nWhat would you like to do?")
            print("1. Received tomorrow’s orders. Would you like to rewrite them?")
        else:
            stock_status = "(Awaiting tomorrow's orders)"
            print("\nWhat would you like to do?")
...
def deduct_today_material_usage(today):
    """
    Deduct raw materials based on today's production before checking stock for tomorrow.
    """
    ...
    if orders_today:
        ...
        new_stock_row = [today.strftime('%Y-%m-%d'), updated_cotton_stock, updated_fibre_stock]
        stock_sheet.append_row(new_stock_row)

        print(f"\nRaw materials deducted for today's production.")


def check_stock_for_tomorrow(tomorrow):
    """
    Check whether stock is adequate for tomorrow's orders.
    """
    orders_tomorrow = get_orders_for_today(tomorrow)
    if not orders_tomorrow:
        return "(Awaiting tomorrow's orders)"
    ...
    if cotton_stock >= cotton_needed and fibre_stock >= fibre_needed:
        return f"(Adequate Stock)"
    else:
        return f"(Insufficient Stock - Cotton: {cotton_stock} meters, Fibre: {fibre_stock} kg)"
...
def request_raw_materials(tomorrow):
    """
    Order raw materials to fulfill tomorrow's orders + 10% reserve.
    """
    orders_tomorrow = get_orders_for_today(tomorrow)
    if not orders_tomorrow:
        print("No orders for tomorrow yet. Cannot order materials.")
        return False
    ...
    # Calculate raw material requirements for tomorrow's orders + 10% reserve
    cotton_needed = (
        (orders_tomorrow['single'] * usage_single['cotton'] +
         orders_tomorrow['double'] * usage_double['cotton'] +
         orders_tomorrow['king'] * usage_king['cotton']) * 1.1
    )
    fibre_needed = (
        (orders_tomorrow['single'] * usage_single['fibre'] +
         orders_tomorrow['double'] * usage_double['fibre'] +
         orders_tomorrow['king'] * usage_king['fibre']) * 1.1
    )

    # Update stock to meet tomorrow's orders + 10%
    new_cotton_stock = cotton_stock + cotton_needed
    new_fibre_stock = fibre_stock + fibre_needed

    stock_sheet = SHEET.worksheet('Material Stock')
    new_stock_row = [tomorrow.strftime('%Y-%m-%d'), new_cotton_stock, new_fibre_stock]
    stock_sheet.append_row(new_stock_row)

    print(f"Ordered raw materials to fulfill tomorrow's orders + 10% reserve.")
    return True
...
def close_day(current_day):
    ...
    deduct_today_material_usage(current_day)
